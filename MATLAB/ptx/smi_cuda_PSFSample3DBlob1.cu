...
__global__ void cuda_PSFSample3DBlob(const double *PX_in, const double *PY_in, const double *PZ_in, const double *PIntensity_in, const double *PBackground_in, const double XYSamPerPix, const double ZSamPerUnit, const int Row, const int Cent, const int *NN, const double *NotPSF, const int NFrames, float *M, int *IndZ1_out, int *IndZ2_out, int *IndZ1CentZ_out, int *IndZ2CentZ_out, int *IndX1Y1_out, int *IndX1Y2_out, int *IndX2Y1_out, int *IndX2Y2_out, double *DecDecZ_out, double *DecDecX_out, double *DecDecY_out)
{
	...
	double IntZ, DecZ, Az, DecDecZ, DecDecX, DecDecY;
...
for (int nn = 0; nn < PixNum; nn++)
	{
		...
		Ind = Ind + 1;
		M[Start + nn] = PIntensity*XYSamPerPix*((1.0f / XYSamPerPix - DecDecY)* (XYSamPerPix*((1.0f / XYSamPerPix - DecDecX)*
			(ZSamPerUnit*((1.0f / ZSamPerUnit - DecDecZ)*NotPSF[Nn*(IndZ1CentZ-1)+IndX1Y1] + DecDecZ*NotPSF[Nn*(IndZ2CentZ-1)+IndX1Y1]))
			+ DecDecX*(ZSamPerUnit*((1.0f / ZSamPerUnit - DecDecZ)*NotPSF[Nn*(IndZ1CentZ-1)+IndX2Y1] + DecDecZ*NotPSF[Nn*(IndZ2CentZ-1)+IndX2Y1])))) + DecDecY*(XYSamPerPix*((1.0f / XYSamPerPix - DecDecX)*
			(ZSamPerUnit*((1.0f / ZSamPerUnit - DecDecZ)*NotPSF[Nn*(IndZ1CentZ-1)+IndX1Y2] + DecDecZ*NotPSF[Nn*(IndZ2CentZ-1)+IndX1Y2]))
			+ DecDecX*(ZSamPerUnit*((1.0f / ZSamPerUnit - DecDecZ)*NotPSF[Nn*(IndZ1CentZ-1)+IndX2Y2] + DecDecZ*NotPSF[Nn*(IndZ2CentZ-1)+IndX2Y2]))))) + PBackground;

	}
 IndZ1_out[PStart] = IndZ1;
 IndZ2_out[PStart] = IndZ2;
 DecDecZ_out[PStart] = DecDecZ;
 DecDecX_out[PStart] = DecDecX;
 DecDecY_out[PStart] = DecDecY;
 IndZ1CentZ_out[PStart] = IndZ1CentZ;
 IndZ2CentZ_out[PStart] = IndZ2CentZ;
 IndX1Y1_out[PStart] = PIndX1Y1;
 IndX1Y2_out[PStart] = PIndX1Y2;
 IndX2Y1_out[PStart] = PIndX2Y1;
 IndX2Y2_out[PStart] = PIndX2Y2;
}