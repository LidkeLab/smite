...
DefaultDisplayParams.RegFileNames = {''; ''};
...
LengthUnitString = ...
    smi_helpers.arrayMUX({'pixel', '$\mu m$'}, DisplayParams.UnitFlag);
TimeDimensionString = ...
    smi_helpers.arrayMUX({'Frame number', 'Time'}, DisplayParams.UnitFlag);
TimeUnitString = ...
    smi_helpers.arrayMUX({'frame', 'second'}, DisplayParams.UnitFlag);
...
if ((~isempty(TRArray(2).IsTransformed)&&TRArray(2).IsTransformed) ...
    ...
    sgtitle(sprintf(...
        ...
        'Parent', FigureHandle, 'FontSize', 8, 'Interpreter', 'none');
else
    sgtitle(sprintf(...
        ['Dimer pair %i, %s ID: %i, %s ID: %i\n', ...
        '%s file: %s\n', ...
...
smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
    TRArray, SMF, DisplayParams, 'ViterbiSequence');
xlabel(PlotAxes, ...
...
hold(PlotAxes, 'on');
smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
    TRArray, SMF, DisplayParams, 'EmissionProbability');
xlabel(PlotAxes, ...
    sprintf('%s (%s)', TimeDimensionString, TimeUnitString), ...
    'Interpreter', 'Latex')
yyaxis(PlotAxes, 'left')
...
hold(PlotAxes, 'on');
smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
    TRArray, SMF, DisplayParams, 'TrajectoryPlot3D');
...
hold(PlotAxes, 'on');
smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
    TRArray, SMF, DisplayParams, 'TrajectoryPlot2D');
xlabel(PlotAxes, sprintf('X (%s)', LengthUnitString), ...
    'Interpreter', 'Latex')
ylabel(PlotAxes, sprintf('Y (%s)', LengthUnitString), ...
    'Interpreter', 'Latex')
...
hold(PlotAxes, 'on');
smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
    TRArray, SMF, DisplayParams, 'XYSeparationPlot');
...
if (isfield(TRArray, 'XRegCorrection') ...
    ...
    hold(PlotAxes, 'on');
    smi_stat.HMM.plotDimerPairInfo(PlotAxes, ...
        TRArray, SMF, DisplayParams, 'RegistrationPlot');
    xlabel(PlotAxes, ...
        sprintf('%s (%s)', TimeDimensionString, TimeUnitString), ...
        'Interpreter', 'Latex')
