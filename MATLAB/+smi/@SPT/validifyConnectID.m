...
[ConnectIDSorted, SortIndices] = sort(ConnectID);
ConnectIDList = unique(ConnectIDSorted);
NLocPerID = groupcounts(ConnectIDSorted);
NLocCumulative = [0; cumsum(NLocPerID)];
NIDs = numel(ConnectIDList);
for nn = 1:NIDs
    % Set all entries with ConnectID==ConnectIDList(nn) equal to nn.
    IndexArray = (1:NLocPerID(nn)) + NLocCumulative(nn);
    ConnectIDSorted(IndexArray) = nn;
end
ConnectID(SortIndices) = ConnectIDSorted;
