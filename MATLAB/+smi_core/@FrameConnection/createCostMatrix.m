...
for mm = 1:NLocalizations
    ...
    for nn = (mm+1):NLocalizations
        ...
        CostBlinkOffThenOn = ...
            -log((1-exp(-KOff))*exp(-KBleach)*exp(-KOn*(DeltaFrame-1))...
            *(1-exp(-KOn)) + (PMiss^(DeltaFrame-1)));
        CostMatrix(mm, nn) = (SeparationCost + CostBlinkOffThenOn) / 2;
        CostMatrix(nn+NLocalizations, mm+NLocalizations) = ...
            (SeparationCost + CostBlinkOffThenOn) / 2;
    end
end
...
for nn = 1:NLocalizations
    ...
    BirthCost = ...
        ...
        + RhoOn(FrameNum(nn)-FrameGapPast)*(1-PMiss)*(PMiss^FrameGapPast));
    DeathCost = -log((1-exp(-KBleach)) ...
        + (1-exp(-KOff))*exp(-KOn*FrameGapFuture) ...
