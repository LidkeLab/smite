...
Success(2) = (all(round(double(FC.SMDCombined.X), 2)==ExpectedX) ...
    && all(round(double(FC.SMDCombined.Y), 2)==ExpectedY) ...
    && all(round(double(FC.SMDCombined.X_SE), 2)==ExpectedXYZ_SE) ...
    && all(round(double(FC.SMDCombined.Y_SE), 2)==ExpectedXYZ_SE) ...
...
FC.FitType = 'XYNBS';
FC.performFrameConnection();
Success(3) = ((round(sum(double(FC.SMDCombined.PSFSigma)), 4)==9.2274) ...
    && all(round(double(FC.SMDCombined.PSFSigma_SE), 4)==0.0042) ...
    && all(round(double(FC.SMDCombined.X), 2)==ExpectedX) ...
    && all(round(double(FC.SMDCombined.Y), 2)==ExpectedY) ...
    && all(round(double(FC.SMDCombined.X_SE), 2)==ExpectedXYZ_SE) ...
    && all(round(double(FC.SMDCombined.Y_SE), 2)==ExpectedXYZ_SE) ...
    && all(round(double(FC.SMDCombined.Photons))==PhotonSum*NFrames) ...
...
FC.FitType = 'XYNBSXSY';
FC.performFrameConnection();
Success(4) = ((round(sum(double(FC.SMDCombined.PSFSigmaX)), 4)==8.5437) ...
...
FC.FitType = 'XYZNB';
FC.performFrameConnection();
Success(5) = (all(round(double(FC.SMDCombined.X), 2)==ExpectedX) ...
    && all(round(double(FC.SMDCombined.Y), 2)==ExpectedY) ...
