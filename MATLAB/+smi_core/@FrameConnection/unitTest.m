...
ExpectedXYZ_SE = round(sqrt(ExpectedVariance), 2);
ExpectedX = round(sum(reshape(SMDSim.X, [], NFrames)/(SigmaNoise^2), 2) ...
    * ExpectedVariance, 2);
ExpectedY = round(sum(reshape(SMDSim.Y, [], NFrames)/(SigmaNoise^2), 2) ...
    * ExpectedVariance, 2);
ExpectedZ = round(sum(reshape(SMDSim.Z, [], NFrames)/(SigmaNoise^2), 2) ...
    * ExpectedVariance, 2);
ExpectedPhotons = PhotonSum * NFrames;
ExpectedBg = Background * NFrames;
ExpectedLogL = LogL * NFrames;
...
Success(2) = baseSuccess(FC.SMDCombined, ...
    ExpectedX, ExpectedY, ExpectedXYZ_SE, ...
    ExpectedPhotons, ExpectedBg, ExpectedLogL, ...
    NFrames, NEmitters);
...
FC.FitType = 'XYNBS';
FC.performFrameConnection();
BaseSuccess = baseSuccess(FC.SMDCombined, ...
    ExpectedX, ExpectedY, ExpectedXYZ_SE, ...
    ExpectedPhotons, ExpectedBg, ExpectedLogL, ...
    NFrames, NEmitters);
Success(3) = (BaseSuccess ...
    && (round(sum(double(FC.SMDCombined.PSFSigma)), 4)==9.2274) ...
    && all(round(double(FC.SMDCombined.PSFSigma_SE), 4)==0.0042));

% Perform the frame-connection procedure for the fit type 'XYNBSXSY' and
% perform various checks to see if it worked correctly.
SMF.FrameConnection.FitType = 'XYNBSXSY'; % not needed, added for emphasis
FC.FitType = 'XYNBSXSY';
FC.performFrameConnection();
BaseSuccess = baseSuccess(FC.SMDCombined, ...
    ExpectedX, ExpectedY, ExpectedXYZ_SE, ...
...
Success(4) = (BaseSuccess ...
    && (round(sum(double(FC.SMDCombined.PSFSigmaX)), 4)==8.5437) ...
    && (round(sum(double(FC.SMDCombined.PSFSigmaY)), 4)==9.5851) ...
    && all(round(double(FC.SMDCombined.PSFSigmaX_SE), 4)==0.0042) ...
    && all(round(double(FC.SMDCombined.PSFSigmaY_SE), 4)==0.0042));

% Perform the frame-connection procedure for the fit type 'XYZNB' and
% perform various checks to see if it worked correctly.
SMF.FrameConnection.FitType = 'XYZNB'; % not needed, added for emphasis
FC.FitType = 'XYZNB';
FC.performFrameConnection();
BaseSuccess = baseSuccess(FC.SMDCombined, ...
    ExpectedX, ExpectedY, ExpectedXYZ_SE, ...
...
Success(5) = (BaseSuccess ...
    && all(round(double(FC.SMDCombined.Z), 2)==ExpectedZ) ...
    && all(round(double(FC.SMDCombined.Z_SE), 2)==ExpectedXYZ_SE));
...
Success(6) = all(ismember(double(IndSMDSum), ...
    ...
    function [BaseSuccess] = baseSuccess(SMDCombined, ...
        ...
        BaseSuccess = ...
            ...
            && all(round(double(SMDCombined.Y), 2)==ExpectedY) ...
            && all(round(double(SMDCombined.X_SE), 2)==ExpectedXYZ_SE) ...
            && all(round(double(SMDCombined.Y_SE), 2)==ExpectedXYZ_SE) ...
            && all(round(double(SMDCombined.Photons))==ExpectedPhotons) ...
            && all(round(double(SMDCombined.LogL))==ExpectedLogL) ...
            && all(round(double(SMDCombined.Bg))==ExpectedBg) ...
