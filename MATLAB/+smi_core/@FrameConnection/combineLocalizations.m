    ...
    case 'XYNBS'
        for ii = 1:NUnique
            ...
            SMDCombined.PSFSigma(ii, 1) = ...
                sum(SMD.PSFSigma(IndexArray)./SMD.PSFSigma_SE(IndexArray).^2) ...
                / sum(1./SMD.PSFSigma_SE(IndexArray).^2);
            SMDCombined.PSFSigma_SE(ii, 1) = ...
                sqrt(1 / sum(1./SMD.PSFSigma_SE(IndexArray).^2));
        end
    case 'XYNBSXSY'
        for ii = 1:NUnique
            IndexArray = (1:NLocPerID(ii)).' + NLocCumulative(ii);
            SMDCombined.PSFSigmaX(ii, 1) = ...
                sum(SMD.PSFSigmaX(IndexArray)./SMD.PSFSigmaX_SE(IndexArray).^2) ...
                / sum(1./SMD.PSFSigmaX_SE(IndexArray).^2);
            SMDCombined.PSFSigmaY(ii, 1) = ...
                sum(SMD.PSFSigmaY(IndexArray)./SMD.PSFSigmaY_SE(IndexArray).^2) ...
                / sum(1./SMD.PSFSigmaY_SE(IndexArray).^2);
            SMDCombined.PSFSigmaX_SE(ii, 1) = ...
                sqrt(1 / sum(1./SMD.PSFSigmaX_SE(IndexArray).^2));
            SMDCombined.PSFSigmaY_SE(ii, 1) = ...
                sqrt(1 / sum(1./SMD.PSFSigmaY_SE(IndexArray).^2));
        end
    case 'XYZNB'
        for ii = 1:NUnique
            IndexArray = (1:NLocPerID(ii)).' + NLocCumulative(ii);
            SMDCombined.Z(ii, 1) = ...
                sum(SMD.Z(IndexArray)./SMD.Z_SE(IndexArray).^2) ...
                / sum(1./SMD.Z_SE(IndexArray).^2);
            SMDCombined.Z_SE(ii, 1) = ...
                sqrt(1 / sum(1./SMD.Z_SE(IndexArray).^2));
        end
end
