...
switch obj.TransformationBasis
    case 'coords'
        ...
        for ii = 2:NFiducials
            ...
            if strcmp(obj.TransformationType, 'lwm')
                % NOTE: The 'lwm' transform can only be applied as an
                %       inverse,  so we want to switch the order of the
                %       "moving" and "fixed" points so as to ensure the
                %       first fiducial never gets transformed (as a
                %       convenience).
                RegistrationTransform{ii} = fitgeotrans(...
                    FinalCoordinates{ii}(:, :, 1), ...
                    FinalCoordinates{ii}(:, :, 2), ...
                    obj.TransformationType, obj.NNeighborPoints);
            elseif strcmp(obj.TransformationType, 'polynomial')
                RegistrationTransform{ii} = fitgeotrans(...
                    FinalCoordinates{ii}(:, :, 2), ...
                    FinalCoordinates{ii}(:, :, 1), ...
