function [PlotFigure] = visualizeTransform(RegistrationTransform, ...
    FrameSize, GridSpacing)
%visualizeTransform creates visuals for channel registration.
% This method will generate some plots that help visualize the channel
% registration and its performance.
%
% INPUTS:
%   RegistrationTransform: RegistrationTransform is a MATLAB tform object
%                          describing the transform to be visualized.
%                          (tform object)
%   FrameSize: Size of the frame (ROI) within which GeomTransform can be
%              applied. (Pixels)(2x1, [YSize, XSize])
%   GridSpacing: Spacing of a grid made across FrameSize.  The grid will
%                consist of points
%                [0:GridSpacing:YSize, 0:GridSpacing:XSize] at which we
%                will apply RegistrationTransform (or interpolate errors,
%                ... depending on the plot being made).
%                (Pixels)(Default = 1)
%
% OUTPUTS:
%   PlotFigures: A MATLAB figure handle for the figure in which the
...
% % % % % if isa(RegistrationTransform, 'images.geotrans.LocalWeightedMeanTransformation2D')
% % % % %     % This is a locally weighted mean transform and should be applied to
% % % % %     % the FixedPointsPaired array.
% % % % %     [XTransformed, YTransformed] = transformPointsInverse(...
% % % % %         RegistrationTransform, XGrid(:), YGrid(:));
% % % % % else
% % % % %     % The transform should be applied to the MovingPointsPaired array.
% % % % %     [XTransformed, YTransformed] = transformPointsInverse(...
% % % % %         RegistrationTransform, XGrid(:), YGrid(:));
% % % % % end
% % % % % XGridTransformed = reshape(XTransformed, size(XGrid));
% % % % % YGridTransformed = reshape(YTransformed, size(YGrid));
...
% % % % % % Plot the heatmap.
% % % % % PlotFigure = figure();
% % % % % PlotAxesTop = subplot(2, 1, 1, 'Parent', PlotFigure);
% % % % % PlotAxesTop.YDir = 'reverse';
% % % % % axis(PlotAxesTop, 'tight');
% % % % % axis(PlotAxesTop, 'equal');
% % % % % hold(PlotAxesTop, 'on');
% % % % % surf(PlotAxesTop, XGrid, YGrid, TransformMagnitude, ...
% % % % %     'EdgeColor', 'none')
